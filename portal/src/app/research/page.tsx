...
export default function ResearchPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-2">Research Memos</h1>
      <p className="text-sm text-[hsl(215,20%,55%)] mb-8">
        Investment committee deep dives
      </p>

      {memos.length === 0 ? (
        <p className="text-[hsl(215,20%,55%)]">No memos yet</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {memos.map((memo) => (
            <Link
              ...
              className="bg-[hsl(222,47%,9%)] border border-[hsl(215,20%,18%)] rounded-lg p-6 hover:border-[hsl(215,20%,30%)] transition-colors"
            >
              ...
              <p className="text-sm text-emerald-400 mt-4">Read memo &rarr;</p>
            </Link>
          ))}
        </div>
      )}
